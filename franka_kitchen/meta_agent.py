...
class MetaAgent(object):
    def __init__(self, env : RoboGymObservationWrapper, goal_list=['microwave'], replay_buffer_size=1000000, max_episode_steps=500):
        # For each task in the goal list, we will have a separate memory and agent
        self.agent_dict = {}
    ...
    def train(self, episodes, batch_size=64, summary_writer_name='meta_agent'):
        ...
        for episode in range(episodes):
            ...
            # Sample two challenges from the list. Sampling three causes negative cross polination.
            for goal in random.sample(list(self.goal_dict.keys()), num_samples):
                ...
                while not done and episode_steps < self.max_episode_steps:
                    ...
                    if self.mem_dict[goal].can_sample(batch_size=64):
                        critic_1_loss, critic_2_loss, policy_loss, ent_loss, prediction_error_loss, alpha = \
                            self.agent_dict[goal].update_parameters(self.mem_dict[goal], batch_size, updates)
                    ...
                    self.mem_dict[goal].store_transition(state, action, reward, next_state, mask)  # Append transition to memory
